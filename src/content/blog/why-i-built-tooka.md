...
faqs:
  - question: 為什麼用 Midjourney、DALL·E 做的社群圖文常常跑版失真？
    answer: 因為生圖模型是用「畫」的，文字對它來說只是圖形——中文字常畫錯、排版無法精準控制。適合做插圖素材，不適合直接產出帶文字的圖文卡片。
...
**直接回答：AI 圖文生成工具分三類——生圖模型直出、Canva 類套版、模板框架＋AI Agent。前兩類的問題分別是「會失真」和「要手動排」，所以我開發了第三類的開源工具 [Tooka](https://github.com/js0980420/tooka)：文字用程式渲染、版型用模板鎖定，從根本上不跑版、不失真。**

## 三類工具的差別

| 類型 | 代表 | 優點 | 致命傷 |
| --- | --- | --- | --- |
| 生圖模型直出 | Midjourney、DALL·E、各種 AI 生圖 | 畫面精美 | 文字是「畫」出來的——中文常出錯、人物會失真、排版不可控 |
| 套版工具 | Canva、簡報軟體 | 版型穩定 | 每張圖手動排，十張輪播排十次，AI 只能幫你想文案 |
| 模板框架＋AI Agent | **Tooka**（開源） | 程式渲染、批量生成、不跑版 | 需要會用 AI Agent（或用我做好的模板直接套） |

## 我在演講現場看到的痛點

我在[台中、雲林場講座](/services/speaking/)教「如何用 AI 製作不跑版、不失真的社群圖文」時發現：學員最挫折的不是不會下指令，而是 AI 生出來的圖**文字是錯的、人物是歪的、每張風格都不一樣**。

根本原因：大家把「生圖模型」當成「排版工具」在用。圖可以用 AI 畫，但**帶文字的圖文卡片應該用程式渲染**——字是真的字、版型是鎖定的模板，這樣才穩定。
